...
def write_3d(path, verts, faces, parameters):
    ...
    def parameter_keys_for_object(params):
        ...
        if shape == "circular":
            keys.update({"diameter", "height", "num_sides", "roof_type", "roof_color_bright", "roof_color_dark", "side_color_bright", "side_color_dark"})
            if roof == "dome":
                keys.update({"dome_layers", "dome_roundness"})
            return keys
    ...
    used_keys = parameter_keys_for_object(parameters)
