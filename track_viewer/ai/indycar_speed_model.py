...
def _corner_speed_mph(
    curvature: float,
    performance: CarPerformance | None = None,
    banking_degrees: float = 0.0,
) -> float:
    """Solve the lateral grip limit, including signed road banking.

    Positive TRK banking means the road falls to the driver's left. A
    positive-curvature (left-hand) corner benefits from this banking, while
    a right-hand corner on the same crossfall is adverse.

    The simple force balance resolves gravity and centripetal acceleration
    into the banked road plane. Its normal load includes the centripetal
    component of banking; speed-dependent aero grip remains the existing
    baseline's additive contribution. This is an engineering approximation,
    not a tire load-sensitivity or suspension model.
    """
    performance = performance or CarPerformance()
    signed_curvature = float(curvature)
    k = abs(signed_curvature)
    if k < 1e-7:
        return MAX_SPEED_MPH

    into_turn_bank = math.radians(
        float(banking_degrees) * (1.0 if signed_curvature > 0 else -1.0)
    )
    bank_cos = math.cos(into_turn_bank)
    bank_sin = math.sin(into_turn_bank)
    ...
    for _ in range(32):
        ...
        # Separate low-speed mechanical grip from speed-dependent aero grip.
        mechanical_g = _LATERAL_G[0, 1]
        aero_g = max(0.0, base_g - mechanical_g)

        # The bank helps through gravity's inward component and through the
        # additional road-normal force generated by centripetal acceleration.
        # An adversely cambered corner has the opposite effects. Do not
        # accept a candidate whose required normal force is negative.
        normal_g = bank_cos + required_g * bank_sin
        required_in_plane_g = required_g * bank_cos - bank_sin
        available_g = performance.safety_factor * performance.cornering_factor * (
            mechanical_g * normal_g + aero_g * performance.aero_factor
        )
        if normal_g > 0.0 and required_in_plane_g <= available_g:
            lo = mph
        else:
            hi = mph
    return lo

def speed_profile_mph(
    points_xy_feet,
    performance: CarPerformance | None = None,
    *,
    ...
    if banking_degrees is None:
        banking = np.zeros(len(points), dtype=float)
    else:
        banking = np.asarray(banking_degrees, dtype=float)
        if banking.shape != (len(points),) or not np.all(np.isfinite(banking)):
            raise ValueError("Banking angles must be finite and match the XY samples.")
