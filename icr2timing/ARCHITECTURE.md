...
* **`reader.MemoryReader`** orchestrates the higher-level decoding via collaborators: `memory_io` (typed reads), `car_state_decoder.decode_car_states` (0x214 telemetry block), and `track_detector.TrackDetector` (WINDY/DOS track detection). It sequences those helpers to read counts, driver names, car numbers, laps, and track metadata, composing immutable `Driver`, `CarState`, and `RaceState` instances while surfacing `ReadError`s for transient issues.【F:icr2_core/reader.py†L1-L188】【F:icr2_core/memory_io.py†L1-L52】【F:icr2_core/car_state_decoder.py†L1-L85】【F:icr2_core/track_detector.py†L1-L79】
* **`memory_io.py`** exposes resilient helpers for reading one or many i32 values from `ICR2Memory`, tolerating byte buffers, raw ints, or iterable responses so callers (including tests) can mock the process boundary easily.【F:icr2_core/memory_io.py†L1-L52】
* **`car_state_decoder.py`** transforms the raw per-car telemetry blob (0x214 ints) into the `CarState` dataclass, handling lap-count clamping, sentinel removal for lap clocks, and preserving all signed integers for custom overlays.【F:icr2_core/car_state_decoder.py†L1-L85】
* **`track_detector.py`** encapsulates the WINDY track-index scanning/caching flow and the DOS/REND32A string read so the logic can be unit-tested independently of `MemoryReader`. It raises `ReadError` via a provided factory when directories/files are missing.【F:icr2_core/track_detector.py†L1-L79】
