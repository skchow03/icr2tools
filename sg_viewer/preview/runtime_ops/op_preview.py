...
class _RuntimeCoreMixin(
    ...
    @property
    def trackside_ahead_referenced_indices(self) -> tuple[int, ...]:
        return tuple(self._trackside_ahead_referenced_indices)
    ...
    def set_trackside_objects(self, objects) -> None:
        ...
        self._trackside_ahead_referenced_indices = tuple(
            index
            for index in getattr(self, "_trackside_ahead_referenced_indices", ())
            if 0 <= index < len(self._trackside_objects)
        )
    ...
    def set_trackside_ahead_referenced_indices(self, indices: tuple[int, ...]) -> None:
        self._trackside_ahead_referenced_indices = tuple(
            dict.fromkeys(
                int(index)
                for index in indices
