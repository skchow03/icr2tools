...
class PreviewRuntime(PreviewRuntimeOps):
    ...
    def __init__(
        ...
        self._trackside_ahead_referenced_indices: tuple[int, ...] = ()
    ...
    @property
    def preview_fsections_by_section(self) -> tuple[tuple[PreviewFSection, ...], ...]:
        """Return the per-section boundary geometry used by the preview."""
        return tuple(tuple(fsections) for fsections in self._fsects_by_section)
    ...
    @property
    def land_object_points_overlay(self) -> tuple[Point, ...]:
        return self._land_object_points_overlay

    def set_land_object_points_overlay(self, points: tuple[Point, ...]) -> None:
        normalized = tuple((float(point[0]), float(point[1])) for point in points)
        if normalized == self._land_object_points_overlay:
            return
        self._land_object_points_overlay = normalized
        self._context.request_repaint()
