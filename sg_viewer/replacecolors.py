...
_COLOR_INDEX_PATTERN = re.compile(r"\bc\s*=\s*<(\d+)>")
_MARK_COLORS_HEADER_PATTERN = re.compile(r"^\s*%%\s*Mark\s+Colors\s*$", re.IGNORECASE)
...
def read_mark_colors(track_file: str | Path) -> "OrderedDict[str, int]":
    """Read palette indices from the ``%%Mark Colors`` section of a .3D file."""
    lines = Path(track_file).read_text(encoding="utf-8", errors="ignore").splitlines()
    in_section = False
    colors: "OrderedDict[str, int]" = OrderedDict()
    for line in lines:
        if not in_section:
            in_section = bool(_MARK_COLORS_HEADER_PATTERN.match(line))
            continue
        if _COMMENT_PATTERN.match(line):
            break
        definition = _COLOR_DEFINITION_PATTERN.match(line)
        if definition is None:
            continue
        index = _COLOR_INDEX_PATTERN.search(definition.group(2))
        if index is not None:
            colors[definition.group(1)] = int(index.group(1))
    if not in_section:
        raise ValueError("The selected .3D file has no %%Mark Colors section.")
    if not colors:
        raise ValueError("The %%Mark Colors section contains no valid mark colors.")
    return colors
...
def replace_color_section_from_indices(
    track_file: str | Path, colors: Mapping[str, int]
) -> int:
    ...
    # Mark colors are generated by TRK23D from track.MRK and must already exist;
    # unlike standard colors, silently inserting them would not connect them to walls.
    mark_colors = {
        name: index for name, index in colors.items() if name not in supported_names
    }
    if mark_colors:
        current_marks = read_mark_colors(path)
        missing = [name for name in mark_colors if name not in current_marks]
        if missing:
            raise ValueError(
                "Could not find imported Mark Color(s) in the .3D file: "
                + ", ".join(missing)
                + ". Generate the .3D with Papyrus TRK23D using a track.MRK "
                "that contains these marks."
            )
        for line_index, line in enumerate(lines):
            match = _COLOR_DEFINITION_PATTERN.match(line.rstrip("\r\n"))
            if match and match.group(1) in mark_colors:
                newline = "\r\n" if line.endswith("\r\n") else "\n"
                name = match.group(1)
                lines[line_index] = (
                    f"{name}: {build_color_value_from_index(mark_colors[name])};{newline}"
                )

    path.write_text("".join(lines), encoding="utf-8")
    return len(replacement_lines) + len(mark_colors)
...
def main() -> None:
    ...
    parser.add_argument(
        "track_file", help="Path to the source track .3D file to update."
    )
    parser.add_argument(
        "colors_file", help="Path to the file containing replacement color definitions."
    )
