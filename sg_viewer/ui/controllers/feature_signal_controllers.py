...
class TsdSignalController:
    ...
    def connect_signals(self) -> None:
        ...
        w.tsd_duplicate_object_button.clicked.connect(
            c._on_tsd_duplicate_object_requested
        )
        w.tsd_remove_selected_object_button.clicked.connect(
            c._on_tsd_remove_selected_object_requested
        )
        ...
        w.tsd_objects_table.itemSelectionChanged.connect(
            c._on_tsd_object_selection_changed
        )
...
class Track3DController:
    ...
    def connect_signals(self) -> None:
        ...
        w.files_trackname_replace_edit.setText(
            h._history.get_template_trackname_files()
        )
        ...
        w.three_d_file_catalog_inspector_button.clicked.connect(
            c._on_three_d_catalog_inspector_requested
        )
        w.three_d_show_section_entries_button.clicked.connect(
            c._on_three_d_show_selected_section_entries
        )
        w.three_d_show_section_object_lists_button.clicked.connect(
            c._on_three_d_show_selected_section_object_lists
        )
        w.three_d_show_section_tsos_button.clicked.connect(
            c._on_three_d_show_selected_section_tsos
        )
        w.three_d_preview_object_list_changes_button.clicked.connect(
            c._on_three_d_preview_selected_object_lists
        )
        w.three_d_apply_object_list_changes_button.clicked.connect(
            c._on_three_d_apply_selected_object_lists
        )
        w.three_d_apply_tso_definitions_button.clicked.connect(
            c._on_three_d_apply_selected_tso_definitions
        )
        ...
        w.three_d_file_fix_in_place_button.clicked.connect(
            c._on_three_d_fix_in_place_requested
        )
        w.three_d_file_select_colors_button.clicked.connect(
            c._on_edit_track3d_colors_requested
        )
        w.three_d_file_apply_colors_button.clicked.connect(
            c._on_three_d_apply_color_replacements_requested
        )
        w.three_d_workflow_save_tso_button.clicked.connect(
            h._trackside_objects_controller._on_tso_write_to_3d_file_requested
        )
        w.three_d_workflow_save_object_lists_button.clicked.connect(
            w.tso_visibility_sidebar._on_save_to_track3d_requested
        )
        w.three_d_workflow_save_detail_lists_button.clicked.connect(
            w.tso_visibility_sidebar._on_save_detail_lists_to_track3d_requested
        )
